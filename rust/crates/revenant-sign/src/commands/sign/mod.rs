...
use revenant_sign_core::constants::{DEFAULT_TIMEOUT_SOAP, ENV_NAME, ENV_PASS, ENV_USER, VERSION};
...
pub(crate) fn sign(app: &App, args: &SignArgs) -> CliResult {
    ...
    let fields = if detached || args.dry_run {
        ...
        revenant_sign_core::api::resolve_signature_fields(&app.store)
    };
...
fn get_credentials(app: &App) -> Result<(String, String), CliError> {
    ...
    prompt_credentials(
        ...
        resolved
            .password
            .as_ref()
            .map(revenant_sign_core::config::Secret::expose),
    )
}
